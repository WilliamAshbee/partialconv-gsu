...
def main():
    ...
    if args.distributed:
        dist.init_process_group(backend=args.dist_backend, init_method=args.dist_url,
                                world_size=args.world_size)#initiate the pytorch distributed process group with the specified backend, init_method, and world size
    #init method is an ip address to look for other process
    #world size is similar to number of processes
    # create model
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader): #enumerate and iterate through the train loader training set values
        ...
        optimizer.step()#optimizer.step updates the value of x using the gradient x.grad. For example, the SGD optimizer performs:
        #e.g. x += -lr * x.grad
